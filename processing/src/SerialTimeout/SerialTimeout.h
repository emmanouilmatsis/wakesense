#ifndef SERIALTIMEOUT_H
#define	SERIALTIMEOUT_H
 ...
 */
class SerialTimeout: private boost::noncopyable
{
	public:
		SerialTimeout();
		 ...
		 * serial device
		 */
		SerialTimeout(const std::string& devname, unsigned int baud_rate,
		...
		/**
		 * Write data
		 * \param data an unsigned char to be sent through the serial device
		 * \throws boost::system::system_error if any error
		 */
		 ...
		 * \param data an unsigned char to be read through the serial device
